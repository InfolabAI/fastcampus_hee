...
NOTE: Gemini CLI Tool Namespacing
---------------------------------
When multiple MCP servers have tools with the same names (e.g., insert, select, update),
Gemini CLI adds prefixes to avoid collisions:
  - 1st loaded server: insert, select, update (no prefix)
  - 2nd loaded server: server_name__insert, server_name__select, ... (prefixed)

This is Gemini CLI behavior, not MCP server behavior.
To use unique names, add explicit names: @mcp.tool(name="b_insert")
"""
# =============================================================================
# Proxy MCP B - MCP 클라이언트와 Proxy Server 사이의 중간 계층
# =============================================================================
# 역할: AI 클라이언트(Gemini/Claude)의 tool 호출을 받아 Proxy Server로 전달
# 보안: JWT 토큰 생성/관리, TLS 인증서 피닝, Infisical 시크릿 관리
# 흐름: AI Client -> [MCP Protocol] -> Proxy MCP -> [HTTPS+JWT] -> Proxy Server
# =============================================================================

import os, json, sys, time, ssl, urllib.request, urllib.error  # 표준 라이브러리
from pathlib import Path  # 파일 경로 처리
from datetime import datetime, timedelta  # JWT 만료 시간 계산
from typing import Optional, Dict, Any, List  # 타입 힌트
from authlib.jose import jwt  # JWT 토큰 생성/검증 라이브러리
from fastmcp import FastMCP  # FastMCP 서버 프레임워크

# -----------------------------------------------------------------------------
# 환경 설정
# -----------------------------------------------------------------------------
PROXY_URL = os.getenv("PROXY_URL", "https://localhost:8443")  # Proxy Server 주소
TENANT = "tenant_b"  # 테넌트 식별자 (JWT 클레임에 포함)
TARGET = "b"  # 라우팅 대상 백엔드 (backend_b)

# JWT_SECRET must come from Infisical
# Infisical에서 주입된 시크릿 - 하드코딩 금지!
...
# 인증서 피닝용 CA 인증서 경로 - MITM 공격 방지
CA_PATH = Path(__file__).parent.parent / "proxy_server" / "certs" / "ca.pem"

# Short-lived token cache (no long-lived storage)
# 토큰 캐시 - 메모리에만 저장 (디스크 저장 금지)
_token_cache = {"token": None, "expires": None, "created": None}

# JWT Configuration
# JWT 설정 - 짧은 수명으로 보안 강화
JWT_LIFETIME_MINUTES = 5  # 토큰 유효 시간: 5분
JWT_REFRESH_BEFORE_MINUTES = 1  # 만료 1분 전에 갱신

# FastMCP server instance
# FastMCP 서버 인스턴스 생성 - AI 클라이언트가 호출할 도구 등록
