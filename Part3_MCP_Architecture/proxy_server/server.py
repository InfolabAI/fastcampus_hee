...
def log_casbin_decision(subject: str, obj: str, action: str, allowed: bool):
    """Log Casbin policy evaluation details"""
    log_entry = {
        "timestamp": datetime.utcnow().isoformat(),
        "type": "CASBIN_DECISION",
        "subject": subject,
        "object": obj,
        "action": action,
        "allowed": allowed,
        "policy_rule": f"p, {subject}, {obj}, {action}" if allowed else None,
        "message": f"Request: {subject}, {obj}, {action} ---> {allowed}"
    }
    print(f"[CASBIN] {json.dumps(log_entry)}")
    with open(BASE_DIR / "proxy_server" / "debug.log", "a") as f:
        f.write(json.dumps(log_entry) + "\n")

def log_jwt_verification(claims: dict, success: bool, error: str = None):
    """Log JWT verification details"""
    now = datetime.utcnow()
    log_entry = {
    ...
    if success and claims:
        log_entry["tenant"] = claims.get("tenant", "unknown")
        if "iat" in claims:
        ...
        if "exp" in claims:
            ...
            log_entry["jwt_expired"] = remaining <= 0
    if error:
        log_entry["error"] = error
    print(f"[JWT_VERIFY] {json.dumps(log_entry)}")
...
def verify_jwt(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    try:
        ...
        log_jwt_verification(claims_dict, success=True)
        return claims_dict
    except Exception as e:
        error_msg = str(e)
        log_jwt_verification(None, success=False, error=error_msg)
...
async def proxy_mcp(target_tenant: str, req: MCPRequest, claims: dict = Depends(verify_jwt)):
    ...
    backend_resource = f"backend_{target_tenant}"

    # Evaluate Casbin policy and log the decision
