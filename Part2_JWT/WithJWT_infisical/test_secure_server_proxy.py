# 필요한 라이브러리와 클래스를 가져옵니다.
import asyncio  # 비동기 작업을 위한 라이브러리
from fastmcp import Client  # FastMCP 클라이언트 생성 클래스
from fastmcp.client.transports import StdioTransport  # STDIO 통신을 위한 클래스


async def test_secure_server_proxy():
    """
    `secure_http_server_proxy.py`를 통해 보안 서버와 통신하는 테스트를 수행합니다.
    이 클라이언트는 JWT 토큰을 직접 다루지 않으며, 모든 인증 처리는 프록시가 담당합니다.
    """

    print("=== 보안 서버 프록시 통신 테스트 ===\n")

    # --- StdioTransport 설정 - --
    # `secure_http_server_proxy.py` 스크립트를 자식 프로세스로 실행하고,
    # 표준 입출력(STDIO)을 통해 통신하는 Transport를 설정합니다.
    # Infisical을 사용하여 JWT_TOKEN 환경변수를 주입합니다.
    transport = StdioTransport(
        command="infisical",  # Infisical을 통해 실행
        # 환경변수 주입 후 Python 실행
        args=["run", "--", "python", "secure_http_server_proxy.py"]
    )

    # transport = StdioTransport(
    #    command="python",  # Infisical을 없이 실행(테스트 용)
    #    # 환경변수 주입 후 Python 실행
    #    args=["secure_http_server_proxy.py"]
    # )

    # --- MCP 클라이언트 생성 ---
    # 위에서 설정한 StdioTransport를 사용하여 MCP 클라이언트를 생성합니다.
    # 이 클라이언트는 이제 `secure_http_server_proxy.py` 프로세스와 통신합니다.
    client = Client(transport)

    try:
        # `async with client:` 구문을 사용하여 클라이언트 연결을 시작하고,
        # 블록이 끝나면 자동으로 연결을 종료합니다.
        async with client:
            print("✅ 보안 서버 프록시 연결 성공!")
            print("   프록시가 JWT 인증을 자동으로 처리합니다.\n")

            # --- 1단계: 사용 가능한 도구 목록 확인 ---
            # 프록시를 통해 실제 서버의 도구 목록을 가져옵니다.
            print("📋 1단계: 사용 가능한 도구 목록")
            tools = await client.list_tools()
            for tool in tools:
                print(f"   - {tool.name}: {tool.description}")
            print()

            # --- 2단계: 보호된 'add' 도구 호출 ---
            # 이 클라이언트는 JWT 토큰 없이 `call_tool`을 호출합니다.
            # 요청을 받은 프록시가 자동으로 JWT 토큰을 헤더에 추가하여
            # 실제 서버로 전달하므로, 인증이 성공적으로 완료됩니다.
            print("📋 2단계: 보호된 add 도구 호출 (JWT 자동 인증)")
            result = await client.call_tool("add", {"a": 42, "b": 58})
            print(f"   ✅ 계산 결과: 42 + 58 = {result}")
            print("   JWT 토큰이 프록시에 의해 자동으로 추가되었습니다.")
            print()

            # --- 3단계: 보호된 'create_greeting' 도구 호출 ---
            # 마찬가지로 프록시가 인증을 투명하게 처리해 줍니다.
            print("📋 3단계: 보호된 create_greeting 도구 호출 (JWT 자동 인증)")
            greeting = await client.call_tool("create_greeting", {"name": "Secure User"})
            print(f"   ✅ 인사말: {greeting}")
            print("   JWT 토큰이 프록시에 의해 자동으로 추가되었습니다.")
            print()

            # --- 4단계: 연속 요청 테스트 ---
            # 여러 번의 요청도 문제없이 처리되는지 확인합니다.
            print("📋 4단계: 연속 요청 테스트")
            for i in range(3):
                result = await client.call_tool("add", {"a": i*10, "b": i*5})
                print(f"   요청 {i+1}: {i*10} + {i*5} = {result} ✅")
            print()

            # --- 최종 결과 및 장점 요약 ---
            print("🎯 프록시 통신 테스트 결과:")
            print("   ✅ 프록시를 통해 JWT 인증이 투명하게 처리됨")
            print("   ✅ 클라이언트는 JWT 토큰을 직접 관리할 필요 없음")
            print("   ✅ 모든 보호된 도구에 정상 접근 가능")
            print("   ✅ 연속 요청이 안정적으로 처리됨")

            print("\n📝 JWT 프록시 패턴의 장점:")
            print("   🔐 클라이언트 코드 단순화: JWT 처리 로직 불필요")
            print("   🔐 중앙화된 인증: 프록시에서 토큰 관리")
            print("   🔐 보안 분리: 비즈니스 로직과 인증 로직 분리")
            print("   🔐 투명한 보안: 기존 MCP 클라이언트 코드 변경 없음")

    except Exception as e:
        # 프록시 연결 또는 통신 중 오류 발생 시 메시지를 출력합니다.
        print(f"❌ 프록시 연결 실패: {e}")
        print("다음을 확인하세요:")
        print("1. secure_http_server.py가 실행 중인지 확인")
        print("2. secure_http_server_proxy.py가 올바르게 구성되었는지 확인")

# 이 스크립트가 직접 실행될 때 main 함수 역할을 하는 부분입니다.
if __name__ == "__main__":
    # asyncio.run()을 사용하여 비동기 함수인 test_secure_server_proxy()를 실행합니다.
    asyncio.run(test_secure_server_proxy())
