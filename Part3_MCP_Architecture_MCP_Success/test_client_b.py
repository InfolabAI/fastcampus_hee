...
async def test_proxy_mcp_b():
    """Test Proxy MCP B tools via FastMCP Client"""
    ...
    # Create stdio transport to launch proxy_mcp_b.py with infisical
    # Use infisical run to inject secrets into subprocess
    transport = StdioTransport(
        command="infisical",
        args=["run", "--", "python", "proxy_mcp_b/proxy_mcp_b.py"]
    )
    ...
    try:
        async with client:
            ...
            result = await client.call_tool("insert", {"name": "test_item_b", "value": "value_from_client_b"})
            ...
            result = await client.call_tool("update", {"id": 1, "value": "updated_value_b"})
            print(f"  Result: {result}")
