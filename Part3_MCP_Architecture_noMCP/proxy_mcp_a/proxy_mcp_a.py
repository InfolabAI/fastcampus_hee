#!/usr/bin/env python3
"""Proxy MCP A - HTTPS client with JWT from Infisical and certificate pinning
...
def get_token():
    ...
    _token_cache["expires"] = datetime.utcnow() + timedelta(minutes=4)  # Refresh 1 min early
    return _token_cache["token"]
...
def call_proxy(method, params, retries=3, backoff=1.0):
    ...
    for attempt in range(retries):
        try:
            ...
            if e.code == 401:  # Token expired, force refresh
